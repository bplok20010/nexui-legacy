define(function(require){
	...
	var panel = Nex.define('Nex.panel.Panel',{
		...
		configs : function(opt, t){
			...
			containerCls : opt.containerCls + ' nex-panel',
			border : true,
	...
	panel.override({
		...
		*/
		_setTools : function(){
			...
			if( opt.toolItems ) {
				//hasTools = true;
				self.addComponent( tools,opt.toolItems );
			}
		...
		_setToolbar : function( items,pos ){
			...
			pos = Nex.inArray( pos, in_pos ) === -1 ? 'top' : pos;
		...
		*/
		...
		setViewSize : function(){
			...
			if( !self.isAutoWidth() ) {
				var w = 0;
				$.each( self._hviews,function(k,el){
					if( !el ) return;	
					w += el.outerWidth();
				} );
			...
			if( !self.isAutoHeight() ) {
				var h = 0;
				$.each( self._vviews,function(k,el){
					if( !el ) return;	
					h += el.outerHeight();
				} );
